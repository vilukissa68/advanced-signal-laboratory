...
The described model was implemented as Pytorch~\cite{pytorch} model. We chose to use Pytorch since it allows for GPU accleration on M1/M2 macbooks via the MPS backend and since we already had some familiarity with it.
...
Since we were limited on time and processing hardware we cannot be sure that this given model is the most performant model that we could have achieved. In our testing the model achieved a accuracy of: 88\% and was sufficiently accurate and fast when we tested it in the completed system.
